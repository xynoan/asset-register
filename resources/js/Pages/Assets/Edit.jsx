...
export default function Edit({ asset, employees }) {
    ...
    const parseCommentsHistory = () => {
        ...
        if (Array.isArray(asset.comments_history)) {
            commentsArray = asset.comments_history;
        } else {
            try {
                const parsed = JSON.parse(asset.comments_history);
                commentsArray = Array.isArray(parsed) ? parsed : [];
            } catch (e) {
                return [];
            }
        }
        ...
        return commentsArray.map(comment => {
            ...
            if (formattedDate && formattedDate !== '') {
                // If date is in a different format, try to format it
                try {
                    const dateObj = moment(formattedDate);
                    if (dateObj.isValid()) {
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="row">
                                <div className="col-md-6">
                                    <div className="mb-3">
                                        <label htmlFor="asset_category" className="form-label">Asset Category <span className="text-danger">*</span></label>
                                        <select
                                ...
                                <div className="col-md-6">
                                    <div className="mb-3">
                                        <label htmlFor="brand_manufacturer" className="form-label">Brand / Manufacturer <span className="text-danger">*</span></label>
                                        <select
